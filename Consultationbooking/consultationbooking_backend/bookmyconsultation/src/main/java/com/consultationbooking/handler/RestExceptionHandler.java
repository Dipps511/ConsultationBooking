package com.consultationbooking.handler;


import com.consultationbooking.controller.ext.ErrorResponse;
...
import com.consultationbooking.exception.AuthorizationFailedException;
import com.consultationbooking.exception.EntityNotFoundException;
import com.consultationbooking.exception.GenericErrorCode;
import com.consultationbooking.exception.InvalidInputException;
import com.consultationbooking.exception.RestException;
import com.consultationbooking.exception.SlotUnavailableException;
import com.consultationbooking.exception.UnauthorizedException;
...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
	...
	//marked as ExceptionHandler for the class SlotUnavailableException
	@ExceptionHandler(SlotUnavailableException.class)
	//created a method handleSlotUnavailableException with return type of ResponseEntity
	public ResponseEntity<ErrorResponse> handleSlotUnavailableException(SlotUnavailableException e) {
		final StringWriter stringWriter = new StringWriter();
		e.printStackTrace(new PrintWriter(stringWriter));
		String message = "Either the slot is already booked or the slot is not available";
		//return http response for bad request with error code and a message
		return ResponseEntity
				.badRequest()
				.body(new ErrorResponse().code("ERR_SLOT_UNAVAILABLE").message(message).rootCause(stringWriter.getBuffer().toString()));
	}
